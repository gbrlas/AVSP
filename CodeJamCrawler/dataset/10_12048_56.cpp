...
{
	int casecnt;
	fin.open("F:\\Preparation\\GCJ\\A-small.in",ios::in);
	fout.open("F:\\Preparation\\GCJ\\A-small.out",ios::out);
	if(!fin.good())
	{
	...
	FRE(e,1,casecnt)
	{
		...
		FR(i,0,n)
		{
			...
			FR(j,0,(vs1.size()-1))
			{
				m.insert(pair<string,string>(vs1[j],vs1[j+1]));
			}
		}
		int ret=0;
		FR(i,0,M)
		{
			...
			int j;
			int cnt=0;
			for(j=0;j<(vs1.size()-1);j++)
			{
				bool done=false;
				for(multimap<string,string>::iterator it=m.lower_bound(vs1[j]);it !=m.upper_bound(vs1[j]); ++it)
				{
					if(it->second == vs1[j+1])
					{
						done=true;
						break;
					}
