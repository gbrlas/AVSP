...
public class Main {
  ...
  void solve() {
    ...
    for(int C=1; C<=T; ++C) {
      ...
      System.out.println("Case #" + C + ": " + solve(W, H, arr));

    }

  }

  int solve(int W, int H, boolean arr[][]) {
    ...
    for(int i=0; i<W; ++i) {
      for(int j=0; j<H; ++j) {
        // part of some building...;
        if( arr[i][j] ) { continue; }
        int node = j * W + i;
        if( j == 0 ) {
          set(graph, S, node, 1, 0);
        }
        // self => self;
        set(graph, node, node + M, 1, 0);
        if( i > 0 && !arr[i - 1][j]) {
          set(graph, node + M, node - 1, 1, 0);
        }
        if( i + 1 < W && !arr[i + 1][j] ) {
          set(graph, node + M, node + 1, 1, 0);
        }
        if( j + 1 < H ) {
          if( !arr[i][j + 1] ) {
            set(graph, node + M, node + W, 1, 0);
          }
        }
        else {
          set(graph, node + M, T, 1, 0);
        }
        if( j > 0 && !arr[i][j - 1] ) {
          set(graph, node + M, node - W, 1, 0);
        }
    ...
    int res[] = flow.minCostFlow(S, T, N, graph, used);
