...
const int MAX_LENGTH = 300000 + 100, DELTA = 200;
const int MOVE[4][2] = {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};

int pathCnt;
char tot[MAX_LENGTH], str[100];
int go[DELTA * 2][DELTA * 2][4], q[DELTA * 2 * DELTA * 2][2], v[DELTA * 2][DELTA * 2];
int top[DELTA * 2][DELTA * 2], down[DELTA * 2][DELTA * 2], LEFT[DELTA * 2][DELTA * 2], RIGHT[DELTA * 2][DELTA * 2];
...
int main()
{
	...
	for (int caseNo = 1; caseNo <= cases; ++caseNo)
	{
		...
		for (int i = 0; i < DELTA * 2; ++i)
			for (int j = 0; j < DELTA * 2; ++j)
				if (v[i][j] == caseNo && (LEFT[i][j] && RIGHT[i][j] || top[i][j] && down[i][j]))
				{
				//	printf("%d, %d\n", i - 500, j - 500);
					++ans;
				}
