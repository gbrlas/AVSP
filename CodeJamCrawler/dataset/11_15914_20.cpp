...
public class BotTrust {
	public static BotTrust bt = new BotTrust();
	/**
	 * @param args
	 * @throws IOException 
	 */
	public static void main(String[] args) throws IOException {
		ArrayList<String> lines = getInput(new File("A-large.in"));
		ArrayList<String> out = new ArrayList<String>();
		for(int i = 0; i < lines.size(); i++){
			if(i ==0)
				continue;
			out.add("Case #"+i+": "+solveSequence(bt.new Sequence(lines.get(i))));
		}
		writeOutput(out, new File("A-large.out"));
	...
	public static int solveSequence(Sequence seq){
		int time = 0;
		Bot botO = bt.new Bot('O');
		Bot botB = bt.new Bot('B');
