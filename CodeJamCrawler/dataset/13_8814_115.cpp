...
public class C {
	...
	static String solve() {
		...
		start = nextPalindromeSafe(start);
	...
	static BigInteger nextPalindromeSafe(BigInteger n) {
		if (nextPalindromeEarly(n).compareTo(n) > -1) {
			return nextPalindromeEarly(n);
		}
		return nextPalindrome(n);
	}
	
	static BigInteger nextPalindromeEarly(BigInteger n) {
		...
		firstHalf = new BigInteger(sb.toString());
