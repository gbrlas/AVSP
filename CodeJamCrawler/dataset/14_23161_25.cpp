...
double solve() {
	...
	int its = 0;
	while(true) {
		//printf("%.5f %.5f %.5f\n", cookies, numCookiesPerSecond, timeElapsed);
		if(cookies >= X)
			break;
		...
		//printf("%.5f\n", (C - cookies) / numCookiesPerSecond );
		//printf("=== %.5f %.5f %.5f\n", timeToWaitToBuyFarms, totalTimeToWaitForCompletionWithBuyingFarms, timeToWaitForCompletion);
