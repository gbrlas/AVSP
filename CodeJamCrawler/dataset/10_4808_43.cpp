...
struct
   ...
   fun isLightOnFast ((n, k) : (int * int)) : bool = 
      let
         val (bigN, bigK) = (Int.toLarge n, Int.toLarge k)
         
         val targetValue : IntInf.int = (IntInf.pow (2, n)) - 1
      
         val (_, modK) = IntInf.divMod (bigK, IntInf.pow (2, n))
      in
         modK = targetValue
      end

   fun isLightOnSlow ((n, k) : (int * int)) : bool = 
      let
   ...
   fun writeFile (rs : string list) = 
      let
         val outFile = TextIO.openOut "output1.txt"
