...
public class B implements Runnable {
	...
	void solve() {
		...
		for (int curTest = 0; curTest < testsNumber; curTest++) {
			...
			System.err.println("Solved test case " + (curTest + 1));
		}
	}

	static String solve(int n, int m, int[][] a) {
//		Vector[][] sum = new Vector[n][m];
		long[][] sumX = new long[n][m];
		long[][] sumY = new long[n][m];
		long[][] bX = new long[n][m];
		long[][] bY = new long[n][m];
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				...
				int y = 2 * j + 1;
				sumD[i][j] = a[i][j];
				bX[i][j] = sumX[i][j] = x * a[i][j];
				bY[i][j] = sumY[i][j] = y * a[i][j];
				if (i > 0) {
					sumX[i][j] += sumX[i - 1][j];
					sumY[i][j] += sumY[i - 1][j];
					sumD[i][j] += sumD[i - 1][j];
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				for (int k = Math.max(ans, 3);; k++) {
					...
					int xc = 2 * i + 1 - k + 1;
					int yc = 2 * j + 1 - k + 1;
					long vX = getSum(sumX, i2, j2, i, j);
					long vY = getSum(sumY, i2, j2, i, j);
					vX -= bX[i2][j2];
					vX -= bX[i][j2];
					vX -= bX[i2][j];
					vX -= bX[i][j];
